...
![lifecycle](E:\授课内容\中公\VUE课程\参考\vue基础\img\vue-smzq2.png)
...
![过渡动画](E:\授课内容\中公\VUE课程\笔记\2022广州Web全栈面授就业班10班\img\过渡动画.png)
...
![构建项目菜单](E:\授课内容\中公\VUE课程\笔记\2022广州Web全栈面授就业班10班\img\构建项目菜单.png)
...
2、当遇到大写字母就改成-小写，俗称烤串写法    eg：webBanner——<web-banner>

3、如果首字母就是大写，那么直接改成小写，不用加-    eg：WebBanner——<web-banner>
...
1、template只能有一个根节点，即其子节点只能有一个标签，但子节点的子节点可以书写多个标签。
2、template选项每次书写相对麻烦，可以借助H5中提供的template标签来书写组件的内容，并将ID绑定给template选项，template仅仅是一个占位符，不会展示在页面中
...
![父传子案例](E:\授课内容\中公\VUE课程\笔记\2022广州Web全栈面授就业班10班\img\父传子案例.png)
...
![插槽](E:\授课内容\中公\VUE课程\笔记\2022广州Web全栈面授就业班10班\img\插槽.png)
...
![非父子组件通信原理](E:\授课内容\中公\VUE课程\笔记\2022广州Web全栈面授就业班10班\img\非父子组件通信原理.png)
...
<keep-alive>是一个内置组件，使用在多组件之间动态切换时，根据特定条件缓存组件实例，保留被包含组件的状态，避免重复渲染DOM，消耗内存获取速度，通常用在组件或路由的缓存。当组件在<keep-alive> 内被切换时，它的 mounted 和 unmounted 生命周期钩子不会被调用，取而代之的是activated(激活钩子函数)和deactivated(失活的钩子函数)，这两个生命周期。
...
3、设置路由出口，打开App.vue,设置路由的出口组件<router-view>。
...
<router-view>是一个组件，路由的出口，用来显示跳转到路由后，路由对应组件的内容。
...
<router-link> 是一个组件，用于设置一个导航链接，切换不同组件内容。 to 属性为目标地址， 即要显示的组件内容。
...
当一个路由需要同时展示多个组件的时候，可以采用<router-view>组件的name属性。
...
·参数是error，跳转被终止的同时会将错误传递给 router.onError() 注册
过的回调。
...
注：不同于前置守卫，后置守卫钩子函数并没有 next 函数，也不会改变导航
本身，使用相对较少。
...
![编程导航和路由守卫不同路由跳转报错](E:\授课内容\中公\VUE课程\笔记\2022广州Web全栈面授就业班10班\img\编程导航和路由守卫不同路由跳转报错.png)
